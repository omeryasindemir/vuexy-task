...
const userListApp = async () => {
  const fetchUsersData = async () => {
    try {
      ...
      localStorage.setItem('users_L_Data', JSON.stringify(data.users))
    } catch (error) {
  ...
  const getUsersData = () => {
    const usersData = JSON.parse(localStorage.getItem('users_L_Data'))
    data.users = usersData
  }

  const hasFetched = localStorage.getItem('isFetchedControl')

  if (!hasFetched) {
    await fetchUsersData()
    localStorage.setItem('isFetchedControl', true)
  }

  console.log(JSON.parse(localStorage.getItem('users_L_Data')))
